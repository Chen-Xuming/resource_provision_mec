...
from codes.min_cost_v2.env.edge_node import EdgeNode
from codes.min_cost_v2.env.user import User
from codes.min_cost_v2.env.service import Service
import numpy as np
...
class Environment:
    ...
    def reset_parameters_about_users(self, user_seed):
        ...
        self.t_price_user_node = user_params_rng.integers(self.minmax_transmission_price[0],
                                                          self.minmax_transmission_price[1] + 1,
                                                          (self.num_user, self.num_edge_node))

        for user in self.user_list:  # type: User
            user.arrival_rate = user_params_rng.integers(self.minmax_arrival_rate[0], self.minmax_arrival_rate[1] + 1)
