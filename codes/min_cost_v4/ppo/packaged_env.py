from codes.min_cost_v4.env.environment import *
import numpy as np
...
class PackagedEnv:
    ...
    """
        重置环境
    """
    def reset(self, user_seed, num_user):
        ...
        self.env.set_users_and_services_by_given_seed(user_seed=user_seed, num_user=num_user)
    ...
    def step(self, action):
        ...
        is_over_capacity = node_r.num_server >= node_r.capacity
        ...
        # 计算cost的增量，以及reward
        new_cost = self.env.compute_cost()
        delta_cost = new_cost - self.cost
        self.cost = new_cost
        # reward += -delta_cost
        # reward += -delta_cost / self.env.total_arrival_rate

        f = (0.5 if is_over_capacity else -0.1)
        reward += (-delta_cost / self.env.total_arrival_rate) * (1 + f)
    ...
    """
        [服务ABR的价格，已分配服务器个数/服务器容量，cost1, cost2]
    ...
    def get_state(self, done=False, mask=None):
        ...
        for node in self.env.edge_node_list:  # type: EdgeNode
            ...
            if done:
                ...
                if not mask_a[node.node_id]:
                    ...
                    original_cost = self.env.compute_cost()
        ...
        # 状态归一化 5维
        states[:, :2] = self.normalize(states[:, :2])
        states[:, 2:3] = self.normalize(states[:, 2:3])
        states[:, 3:4] = self.normalize(states[:, 3:4], normalize_cost=True)
    ...
    def initialize(self):
        ...
        self.cost = self.env.compute_cost()
