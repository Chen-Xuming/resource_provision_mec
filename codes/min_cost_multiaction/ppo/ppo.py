...
class Actor(nn.Module):
    ...
    def __init__(self, args, env):
        ...
        self.gcn = GeneralConv(env.node_state_dim, args.node_embedding_size, 1)
        self.gcn2 = GeneralConv(env.node_state_dim, args.node_embedding_size, 1)
        ...
        self.fc4 = nn.Linear(args.hidden_dim // 2, env.num_edge_node * 2)    # 输出的维度是 num_edge_node * 2，随后用 torch.split() 分成2份。
...
class PPO:
    ...
    def evaluate(self, graph_s, user_s):
        a_prob = self.actor(graph_s, user_s, is_batch=False)
        split_probs = torch.split(a_prob, [self.actor.env.num_edge_node, self.actor.env.num_edge_node])
        a0 = np.argmax(split_probs[0].detach().numpy())
        a1 = np.argmax(split_probs[1].detach().numpy())
        return [a0, a1]
    ...
    def choose_action(self, graph_state, user_state):
        ...
        action_size = self.actor.env.num_edge_node
        logits = self.actor(graph_state, user_state)
        split_logits = torch.split(logits, [action_size, action_size])
        multi_categoricals = [Categorical(logits=lg) for lg in split_logits]
    ...
    def update(self, replay_buffer, total_steps):
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                logits = self.actor(data_mini_batch, u[index], is_batch=True)
                split_logits = torch.split(logits, [self.actor.env.num_edge_node, self.actor.env.num_edge_node], dim=1)
                multi_categoricals = [Categorical(logits=lg) for lg in split_logits]
    ...
    def lr_decay(self, total_steps):
        ...
        # self.lr_a = self.lr_a * 0.995
        # self.lr_c = self.lr_c * 0.995
        # for p in self.optimizer_actor.param_groups:
        #     p['lr'] = self.lr_a
        # for p in self.optimizer_critic.param_groups:
        #     p['lr'] = self.lr_c

        # print("step: {} / {}, lr = {}".format(total_steps, self.max_train_steps, self.lr_a))
