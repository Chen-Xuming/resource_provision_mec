"""
    考虑优化整个交互时延的版本(暂不考虑关联关系的决策)
    交互时延 = 传输时延 + 排队时延 + 处理时延

    在原算法的基础上，记录目标：min |delay-i - delay-j|
"""
...
class Algorithm:
    ...
    """
        ...
        2. min_delay
        3. avg_delay
        4. max_delay - min_delay
    """
    def get_min_difference_result(self):
        cost = self._env.compute_cost()
        self._result_info = self._env.collect_interaction_delay_info()
        print("final interaction delay information: (cost = {}, budget = {})".format(self._cost, self._env._cost_budget))
        for key, value in self._result_info.items():
            print("{} : {}".format(key, value))
        assert cost <= self._env._cost_budget

    def get_result_dict(self):
        self._result_info["running_time"] = self._running_time
        return self._result_info

IF_DEBUG = False
IF_SAVE_GREEDY = False
debug_save_dir = "xx.txt"
if IF_SAVE_GREEDY:
    file = open(debug_save_dir, "w")
    ...
    def set_num_server(self):
        ...
        while self._cost < self._env._cost_budget:
            ...
            if IF_DEBUG:
                ...
                indices = (a, q, (r, sub_r))
                print("service chain: {}".format(indices))
                print("max_delay: {}, user_pair: ({}, {})".format(max_delay, user_i._id, user_j._id))
                print("services: {}".format(services))
                if IF_SAVE_GREEDY:
                    file.write("-------------- iteration {} --------------\n".format(self.iteration))
                    file.write("max_delay: {:.6f}   constant: {:.6f}\n".format(max_delay, max_delay - self._env.compute_queuing_delay(user_i._id, user_j._id)))
                    file.write("user_pair: ({}, {})\n".format( user_i._id, user_j._id))
                    file.write("service chain: {}\n".format(indices))
            ...
            if services[k]["price"] > self._env._cost_budget - self._cost:
                ...
                if IF_DEBUG:
                    if max_utility:
                        ...
                        if IF_SAVE_GREEDY:
                            file.write("service_to_add_a_server: {}\n\n".format(services[k]))
                break
        return max_utility
...
class delay_balance(Algorithm):
    ...
    """
        利用原有的greedy算法将所有时延降低到阈值以下
    """
    def reduce_delays_to_threshold(self):
        self.get_initial_max_interaction_delay()
        _, _, count_large_delay = self.get_max_delay_and_min_delay()
        while self._cost < self._env._cost_budget and count_large_delay > 0:
            max_delay, user_i, user_j = self._env.get_max_interaction_delay()
    ...
    """
        reduce_delays_to_threshold() 的辅助函数
    """
    def get_max_utility(self, services):
        services = sorted(services, key=lambda services: services["reduction"] / services["price"], reverse=True)
        max_utility = None
    ...
    def set_num_server(self):
        ...
        """
            step 1: 使用原 greedy 算法将所有交互时延降到阈值以下
                    使用这个算法的原因是它快速且节约成本
        """
        self.DEBUG("--------------------- step 1 ---------------------------")
        self.reduce_delays_to_threshold()

        self.DEBUG("cost after step 1: {} / {}".format(self._cost, self._env._cost_budget))

        """
            step 2: 对最大时延做对最小时延影响最小的调整
                    第 1 阶段：在不影响当前 T_min 的前提下，继续降低 T_max
                    第 2 阶段：T_min的资源归还 ———— 若归还服务器之后，T_max仍不变，那么就归还一个服务器，否则结束
        """
        self.DEBUG("--------------------- step 2 ---------------------------")
        finish = False
        while not finish:
            ...
            self.DEBUG("--------------------- step 2-1 ---------------------------")
            while self._cost < self._env._cost_budget and not finish:
                ...
                finish = self.allocate_for_min_influence_and_max_reduction(min_delay_reductions, max_delay_reductions)
                self._cost = self._env.compute_cost()
            ...
            while not finish:
                max_delay, min_delay, count_large_delay = self.get_max_delay_and_min_delay()
                min_service_chain = self._env.get_service_index(min_delay[0][0], min_delay[0][1])
                service_a = self._env._service_A[min_service_chain[0]]
                service_r = self._env._service_R[min_service_chain[2]][min_service_chain[3]]

                increment_a = service_a.delay_increment_when_reduce_a_server()
                increment_r = service_r.delay_increment_when_reduce_a_server()

                candidates = [(increment_a, service_a), (increment_r, service_r)]
                sorted(candidates, key=lambda x: x[0], reverse=True)
                quit_flag = True
                for candidate in candidates:
                    if candidate[0] == 0 or candidate[0] + min_delay[1] > max_delay[1]:
                        continue
                    candidate[1].update_num_server(candidate[1]._num_server - 1)        # 尝试归还一个服务器
                    current_max_delay, _, _ = self._env.get_max_interaction_delay()
                    if current_max_delay > max_delay[1]:    # 如果归还之后，最大时延比原来的还大，怎取消归还
                        candidate[1].update_num_server(candidate[1]._num_server + 1)
                    else:
                        return_count += 1
                        quit_flag = False
                if quit_flag:
                    break
            self.DEBUG("return_count = {}".format(return_count))
            if return_count == 0:
                break
...
IF_SAVE_PULP = False
if IF_SAVE_PULP:
    file = open("min_max/result/pulp_solution_7252836.txt", "w")
class min_max_pulp(Algorithm):
    ...
    def services_to_dict(self):
        ...
        self._services_dict[("q", 0)] = self._env._service_q
    ...
    def set_num_server(self):
        ...
        for user_i in range(self._env._num_user):
            for user_j in range(self._env._num_user):
                a, q, r, sub_r = self._env.get_service_index(user_i, user_j)
                ...
                model += self._env.compute_interaction_delay(user_i, user_j) - \
                         ...
                         sum([self._delay_reductions_each_service[("q", q)][j] * x[("q", q)][j] for j in
                              range(len(self._delay_reductions_each_service[("q", q)]))]) - \
...
IF_SAVE_EQUAL_WEIGHT = False
if IF_SAVE_EQUAL_WEIGHT:
    file = open("min_max/result/equal_weight_solution.txt", "w")
class min_max_equal_weight(Algorithm):
    def __init__(self, env):
        Algorithm.__init__(self, env)
        self._service_list = {
            "a": self._env._service_A,
            "q": self._env._service_q,
            "r": self._env._service_R
        }
        self.iteration = 1

    """
        for_surrogate_relaxation=Ture ====> 用equal_weight算法估计替代松弛算法的a值
    """
    def set_num_server(self, for_surrogate_relaxation=False):
        self._solutions = []
        ...
        while self._cost < self._env._cost_budget:
            ...
            for key in self._weight.keys(): # key = (type, id)
                ...
                services.append({
                    ...
                    "delay_reduction": service.reduction_of_delay_when_add_a_server(),
            ...
            if max_utility is not None:
                if IF_DEBUG:
                    if IF_SAVE_EQUAL_WEIGHT:
                        ...
                        max_delay, user_i, user_j = self._env.get_max_interaction_delay()
                        a, q, r, sub_r = self._env.get_service_index(user_i, user_j)
                        service_chain = [a, q, (r, sub_r)]
                        file.write("max_delay: {:.6f}\n".format(max_delay))
                        file.write("service_chain: {}\n".format(service_chain))
                        file.write("type: {}   id: {}\n".format(max_utility["type"], max_utility["id"]))
                        file.write("reduction: {:.6f}\n".format(max_utility["delay_reduction"]))
                        file.write("weight: {:.6f}   price: {}\n".format(max_utility["weight"], max_utility["price"]))
                        file.write("reduction * weight / price = {:.8f}\n\n".format(max_utility["reduction"] / max_utility["price"]))
                ...
                if s_type == "a" or s_type == "b":
                    ...
                    self._service_list["q"].update_num_server(self._service_list["q"]._num_server + 1)
                else:
        ...
        if IF_DEBUG and IF_SAVE_EQUAL_WEIGHT:
            file.close()

        if not for_surrogate_relaxation:
            self.get_min_difference_result()
            self.get_running_time()

        # 为替代松弛算法估计a值
        if for_surrogate_relaxation:
            max_delay, _, _ = self._env.get_max_interaction_delay()
            upper_bound = max_delay

            # 计算 min-sum
            lower_bound = 0
            weight = 1 / len(self._env._interactive_delay_list)
            for key, value in self._env._interactive_delay_list.items():
                user_i, user_j = value["users"]
                delay = self._env.compute_interaction_delay(user_i, user_j)
                lower_bound += delay * weight

            a = (upper_bound - lower_bound) / lower_bound
            return a
    ...
    def compute_weight(self):
        ...
        numerator = len(self._env._interactive_delay_list)

        if IF_DEBUG:
            print("unit weight = {:.6f}".format(1 / numerator))
...
DEBUG_SURROGATE_RELAXATION = False
class min_max_surrogate_relaxation(Algorithm):
    ...
    def set_num_server(self):
        ...
        # equal_weight = min_max_equal_weight(self._env)
        # high_a = equal_weight.set_num_server(for_surrogate_relaxation=True)
        #
        # # 线性搜索
        # debug = True
        # a = high_a
        ...
        #
        # max_delay, user_i, user_j = self._env.get_max_interaction_delay()
        # self._max_interaction_delay = convergence_max_interaction_delay if convergence_max_interaction_delay != -1 else max_delay
        # # self.surrogate(convergence_a)
        # if debug:
        #     print("Convergence a = {}".format(convergence_a))
        #     print("max_interaction_delay = {}".format(self._max_interaction_delay))
        ...
        low_a = 0
        equal_weight = min_max_equal_weight(self._env)
        high_a = equal_weight.set_num_server(for_surrogate_relaxation=True) * 2
        mid = 0

        debug = True
        if debug:
            print("initial high_a = {}".format(high_a))
        ...
        max_delay, user_i, user_j = self._env.get_max_interaction_delay()
        print("final_max_delay = ", max_delay)
    ...
    """
        判断是否收敛：
        如果constraint个数超过 1/x 的时延总数，就认为给定的a值下不可以收敛。
    """
    def surrogate(self, a):
        ...
        while True:
            if len(self._constraints) > len(self._env._interactive_delay_list) / 5:
                convergence = False
                break
    ...
    def line_search(self):
        ...
        self.compute_min_sum_allocation_solution(lambda_k)
    ...
    def get_services_for_utility(self, lambda_k):
        ...
        for constraint in self._constraints:
            ...
            if ('q', 0) in services.keys():
                services[('q', 0)]['reduction'] += self._env._service_q.reduction_of_delay_when_add_a_server() * \
                                                    constraint['lambda'] * weight
            else:
                services[('q', 0)] = {
                    'type': 'q',
                    'index': 0,
                    'reduction': self._env._service_q.reduction_of_delay_when_add_a_server() * constraint[
                        'lambda'] * weight,
                    'price': self._env._service_q._price
                }

            if ('r', constraint['r']) in services.keys():
                services[('r', constraint['r'])]['reduction'] += self._env._service_R[constraint['r'][0]][constraint['r'][1]].reduction_of_delay_when_add_a_server() * \
                                                                 constraint['lambda'] * weight
            else:
                services[('r', constraint['r'])] = {
                    ...
                    'reduction': self._env._service_R[constraint['r'][0]][
                                     constraint['r'][1]].reduction_of_delay_when_add_a_server() * constraint['lambda'] * weight,
    ...
    def compute_min_sum_allocation_solution(self, lambda_k, debug=False):
        ...
        while self._cost < self._env._cost_budget:
            ...
            if debug:
                ...
                if max_utility:
                    print("max_utility = {{type: {}, index: {}, reduction: {:.7f}, price: {}}}".format(
                        max_utility["type"], max_utility["index"],
            ...
            if max_utility:
                ...
                elif max_utility['type'] == 'q':
                    max_utility = self._env._service_q
        ...
        elif service_over_budget["type"] == "q":
            service_over_budget = self._env._service_q
