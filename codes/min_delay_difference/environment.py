"""
    1. 交互时延对只有 n 组，对于每个用户 user-i，只考虑时延 (user-i, user-i)
    2. 不去重
"""
...
class Service:
    ...
    def delay_increment_when_reduce_a_server(self):
        num_server = self._num_server - 1
        if num_server * self._service_rate <= self._arrival_rate:
            return 0
...
class Env:
    def __init__(self, config, rng, seed_sequence):
        ...
        # 当考虑整个交互时延时，初始化这个
        self.get_interactive_chain_list()   # user_i ~ user_i 不去重
    ...
    def initialize_service(self):
        ...
        self._tx_a_q = self._rng.integers(2, 4 + 1, self._num_service_a)
        self._tx_q_r = self._rng.integers(2, 4 + 1, self._num_service_r)
    ...
    """
        获取所有 user_i 到 user_i 的服务链
        无去重
    """
    def get_interactive_chain_list(self):
        self._interactive_delay_list = {}
        for i in range(self._num_user):
            delay_index = i
            a, q, r, sub_r = self.get_service_index(i, i)
            service_set = [a, q, (r, sub_r)]
            self._interactive_delay_list[delay_index] = {
                "service_chain": service_set,
                "users": (i, i),
                "new_index": delay_index
            }
    ...
    def get_max_interaction_delay(self):
        ...
        for key, value in self._interactive_delay_list.items():
            user_i = value['users'][0]
            user_j = value['users'][1]
    ...
    """
    def collect_interaction_delay_info(self):
        delays = []
        for key, value in self._interactive_delay_list.items():
            user_i = value['users'][0]
            user_j = value['users'][1]
            delay = self.compute_interaction_delay(user_i, user_j)
            delays.append(delay)

        assert len(delays) == self._num_user

        max_delay = max(delays)
        min_delay = min(delays)
        avg_delay = sum(delays) / len(delays)
        min_max_difference = max_delay - min_delay
        return {"max_delay": max_delay, "min_delay": min_delay, "avg_delay": avg_delay, "min_max_difference": min_max_difference}

    """
        获取当前交互时延列表，排倒序
    """
    def get_sorted_interaction_delay_list(self):
        delays = {}
        for key, value in self._interactive_delay_list.items():
            user_i = value['users'][0]
            user_j = value['users'][1]
            delay = self.compute_interaction_delay(user_i, user_j)
            delays[(user_i, user_j)] = delay
