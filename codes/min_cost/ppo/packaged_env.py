...
class PackagedEnv:
    ...
    def step(self, action):
        ...
        if not self.is_tx_tp_satisfied(user, node_a, node_r):
            ...
            # reward += len(self.assigned_users) / self.env.num_user
    ...
    def get_state(self):
        ...
        for node in self.env.edge_node_list:    # type: EdgeNode
            ...
            service_A_arrival_rate = 0
            if (None, "A") in node.service_list:
                service_A_arrival_rate = node.service_list[(None, "A")].arrival_rate

            node_state.append(service_A_arrival_rate)      # service_A 的到达率，如果本站点还没有serviceA，则为0
            ...
            max_delay_UA = 0
            max_delay_RU = 0
            service_A = None if (None, "A") not in node.service_list else node.service_list[(None, "A")]
            for u in self.assigned_users:    # type: User
                if u.service_A is service_A:
        ...
        graph = Data(x=torch.tensor(node_state_list, dtype=torch.float),
        ...
        while max_delay > self.env.delay_limit:
            ...
            while cur_delay > self.env.delay_limit:
                ...
                services.sort(key=lambda x: x.reduction_of_delay_when_add_a_server() /
                                            (x.price if self.env.edge_node_list[x.node_id].num_server <
                                                        self.env.edge_node_list[x.node_id].capacity else x.extra_price), reverse=True)
