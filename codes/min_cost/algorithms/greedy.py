...
class GreedyAssignmentAllocation(BaseAlgorithm):
    ...
    def deploy_for_users(self):
        for user in self.env.user_list:     # type: User
            ...
            while len(candidate_pairs) > 0:
                ...
                self.allocate_for_delay_limitations(cur_operation_records)
    ...
    """
        将user 加入到 service_A
    """
    def attach_user_to_serviceA(self, user: User, service_a: Service, records: list):
        ...
        self.record(operation_records=records, op="attach", service=service_a, edge_node=None, user=user)
        ...
        if num_server_for_stab > origin_num_server:
            ...
            self.record(operation_records=records, op="allocation_for_A", service=service_a, edge_node=edge_node,
                        num=num_server_for_stab-origin_num_server,
                        num_extra=extra_num_server)
        else:
    ...
    """
    def allocate_for_delay_limitations(self, records: list):
    ...
    """
        ...
        ("allocation_for_A", service, edge_node, num, num_extra)
        ("attach", service, user)
    """
    def record(self, operation_records: list, op: str, service: Service, edge_node: EdgeNode = None, num: int = None,
        ...
        elif op == "allocation_for_init" or op == "allocation_for_limit" or op == "allocation_for_A":
            operation_records.append((op, service, edge_node, num, num_extra))
        elif op == "attach":
            operation_records.append((op, service, user))
        else:
    ...
    def undo_operations(self, operation_records: list):
        while len(operation_records) > 0:
            ...
            if op == "allocation_for_init":
            ...
            elif op == "allocation_for_limit" or op == "allocation_for_A":
                op, service, edge_node, num, num_extra = operation_records[-1]
                ...
                if service.service_type == "A" and len(operation_records[-1]) == 4:
                    user = operation_records[-1][3]
                    user.service_A = None

            elif op == "attach":
                op, service, user = operation_records[-1]
                service.arrival_rate -= user.arrival_rate
                service.update_num_server(service.num_server, service.num_extra_server, update_queuing_delay=True)     # 这一步是为了更新排队时延的
                user.service_A = None
    ...
    def commit(self, operation_records: list):
        for record in operation_records:
            ...
            if op == "allocation_for_init" or op == "allocation_for_limit" or op == "allocation_for_A":
                ...
                service.extra_price = edge_node.extra_price[service.service_type]
                edge_node.service_list[(service.user_id, service.service_type)] = service
                if service.service_type == "A" and len(record) == 4:
                    user = record[3]
                    user.service_A = service

            elif op == "attach":
                op, service, user = record
                service.arrival_rate += user.arrival_rate

                # 这里需要更新排队时延，因为到达率发生了变化
                # 能否只在 commit "allocation_for_A" 的时候更新？
                # 不行，因为”attach“之后不一定是"allocation_for_A"
                service.update_num_server(service.num_server, service.num_extra_server, update_queuing_delay=True)
